...
<div class="ch tall"><canvas id="c_rums"></canvas></div></div>
</div>
...
{callout(f'{sev_pct_v}% of bus-involved crashes are fatal or serious, against {all_sev_v}% for all NSW crashes. Severity rises with the speed zone, from 22% at 50 km/h to between 33% and 37% in the 80–110 km/h zones. Most of the severe outcomes sit in two situations: pedestrian conflict in dense centres, and higher-speed running on the regional network.')}
''')}
...
new Chart(document.getElementById('c_rums'), {{type:'bar',
  ...
  options:{{indexAxis:'y', maintainAspectRatio:false, ...noleg,
    ...
    scales:{{x:{{beginAtZero:true, grid:GRID, title:{{display:true,text:'% fatal or serious',font:{{size:10}}}}}}, y:{{grid:NOGRID}}}}}}}});
